...
class plgAuthenticationNavajo extends JPlugin
{
    ...
    function onAuthenticate( $username, $password, &$response )
    {
        ...
        $n = NavajoClient::callInitService("vla/authentication/clubsite/InitUserAuthenticate");
        if(is_null($n)) {
            $response->status   = JAUTHENTICATE_STATUS_FAILURE;
            $response->error_message = 'Could not authenticate (1a)';
            return;
        }
        $usr = $n->getAbsoluteProperty('/AuthenticationData/UserName');
        if(!isset($usr) || !is_object($usr)) {
            $response->status   = JAUTHENTICATE_STATUS_FAILURE;
            $response->error_message = 'Could not authenticate (1b)';
            return;
        }

        $usr->setValue(strtoupper($username['username']));
        $n->getAbsoluteProperty('/AuthenticationData/PassWord')->setValue(strtoupper($username['password']));
        $n->getAbsoluteProperty('/AuthenticationData/ClubIdentifier')->setValue($navajoUsername);
        $n2 = NavajoClient::processNavajo("vla/authentication/clubsite/ProcessLoginClubsiteUser",$n);
                
        $resultProp = $n2->getAbsoluteProperty('/UserData/IsAuthenticated');
        $result = 'false';
        if(isset($resultProp)) {
            $result = $resultProp->getValue();
            $role = $n2->getAbsoluteProperty('/UserData/UserRole')->getValue();
        }
        if( $result == 'true' && ( $role == 'CLUBSITEUSER' OR $role == 'CLUB_ADMIN' ) ) {
            if ($role != 'CLUB_ADMIN') {
            ...
            $response->status   = JAUTHENTICATE_STATUS_FAILURE;
            $response->error_message = 'Could not authenticate (2)';
        }
    }
}
?>
